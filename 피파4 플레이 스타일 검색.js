...
function find_match_id(aI, start, end) {
  //user accessid를 파라미터로 공식경기 매치기록 조회(닉네임을 통해 조회가 불가능)
  var doc = JSON.parse(
    Jsoup.connect(
...
function find_match_info(match, aI) {
  //조회된 매치id를 이용하여 매치정보를 받아온다. 해당 Json 데이터에는 참 재미있는 정보가 많았고, 재미요소와 깔끔하게 보이기 위해 골대맞춘 횟수만 넣었다.
  var i;
  var doc = JSON.parse(
...
function response(
  ...
  globalMutex.Lock = (Unlock) => {
    if (msg.startsWith("!유형")) {
      ...
      myTotalPossession = 0;
      try {
        //만약 본인 혹은 상대방이 닉네임 변경했을 경우 matchInfo에서 nickname을 인덱스로 찾을 수 없어, 예외처리를 하였다. catch이후 빠져나가기때문에, try-catch문을 두번 돌렸는데, 불완전하다.
      ...
      replier.reply(
        "<" +
          ...
          "\n" +
          "평균 점유율 : " +
          (myTotalPossession / len).toFixed(2) +
          "%\n" +
          "헤딩슛 시도 : " +
          (myTotalHeadingShoot / len).toFixed(2) +
          "번\n" +
          "헤딩골 : " +
          (myTotalHeadingGoal / len).toFixed(2) +
          "골\n" +
          "성공률 : " +
          ((myTotalHeadingGoal / myTotalHeadingShoot) * 100).toFixed(2) +
          "%\n" +
          "페널티 안 슈팅 시도 : " +
          (myTotalInPenaltyShoot / len).toFixed(2) +
          "번\n" +
          "페널티 안 골 : " +
          (myTotalInPenaltyGoal / len).toFixed(2) +
          "골\n" +
          "성공률 : " +
          ((myTotalInPenaltyGoal / myTotalInPenaltyShoot) * 100).toFixed(2) +
          "%\n" +
          "페널티 밖 슈팅 시도 : " +
          (myTotalOutPenaltyShoot / len).toFixed(2) +
          "번\n" +
          "페널티 밖 골 : " +
          (myTotalOutPenaltyGoal / len).toFixed(2) +
          "골\n" +
          "성공률 : " +
          ((myTotalOutPenaltyGoal / myTotalOutPenaltyShoot) * 100).toFixed(2) +
          "%\n"
      );
    }
    Unlock();
