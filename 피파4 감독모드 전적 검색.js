const scriptName = "피파4 developer open api 감독모드 공식경기 전적 검색";
...
function find_match_id(aI,start,end){ //user accessid를 파라미터로 공식경기 매치기록 조회(닉네임을 통해 조회가 불가능)
  var doc=JSON.parse(Jsoup.connect("https://api.nexon.co.kr/fifaonline4/v1.0/users/"+aI+"/matches?matchtype=52&offset="+start+"&limit="+end)
  .header("Authorization","myNexonKey")
  .ignoreContentType(true)
  .get().text())
  return doc
...
function response(room, msg, sender, isGroupChat, replier, imageDB, packageName) {
  if (msg.startsWith("!감모매치")) {
    search = msg.substr(5).trim();
